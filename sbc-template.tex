...
\begin{resumo}
  A crescente dependência de sistemas conectados elevou significativamente a exposição de organizações a ameaças cibernéticas, especialmente em aplicações web e APIs. Vulnerabilidades classificadas pelo OWASP Top 10 continuam entre as mais exploradas por agentes mal-intencionados, tornando essencial a compreensão de seus impactos e a adoção de práticas de desenvolvimento seguro. Este trabalho tem como objetivo analisar as vulnerabilidades listadas no OWASP Top 10 (2021) por meio de um Pentest realizado no site institucional Atitus. Com autorização prévia da instiuição, serão relizados as etapas de um pentest com base no PTES, fazendo o reconhecimento, coleta de informações, exploração, e um relátorio com oque foi identificado atráves dos testes. A metodologia inclui etapas de reconhecimento, varredura, exploração e validação utilizando ferramentas amplamente utilizadas no contexto de cyber-segurança. Os resultados esperados envolvem demonstrar etapas de um processo de pentest, evindenciar testes realizados e respectivos resultados e citar melhorias sobre o ambiente web da plataforma, assim como reportar eventuais vulnerabilidades ecnontradas. A pesquisa busca contribuir para a formação de desenvolvedores e analistas de segurança, reforçando a importância de avaliações contínuas e metodologias estruturadas de pentest na construção de aplicações robustas.
...
Diante da relevância do tema, o objetivo geral deste trabalho é identificar, explorar e analisar vulnerabilidades presentes nas listas do OWASP Top 10 por meio através de um Pentest a ser realizado no site institucional da faculdade Atitus (atitus.edu.br).

Essa abordagem busca demonstrar etapas de um teste de penetração (Pentest), desde o reconhecimento até o relatório final, assim como evindenciar técnicas exploratórias das principais e mais comuns falhas de segurança, listadas no OWASP TOP 10.
...
Para a validação e análise das vulnerabilidades listadas no OWASP top 10, será relizado um pentest no site institucional Atitus. Com autorização prévia, serão realizados os passos do Pentest com o intuito de colocar em prática as abordagens previamente discutidas, tendo como base o OWASP TOP 10 e o padrão PTES para o Pentest.

Com autorização prévia o pentest será relizado e as evidencias serão descritas no presente documento. Não serão utilizados métodos de Dos, pois podem gerar instablidade no site. O dominio a ser atacado será o www.atitus.edu.br.

\subsection{Reconhecimento}

Nesta etapa, serão empregadas ferramentas de reconhecimento e coleta de informações com o objetivo de identificar características do alvo, como sistema operacional do host, servidores DNS, serviços expostos e possíveis vetores de ataque. Essa fase é fundamental para o mapeamento inicial da superfície de ataque da aplicação em análise.

O sistema operacional escolhido é o Kali Linux, amplamente adotado em testes de segurança por oferecer diversas ferramentas especializadas para atividades de pentest e análise de vulnerabilidades. Entre essas ferramentas, destaca-se o Nmap, utilizado para descoberta de hosts, identificação de portas abertas, serviços ativos e tecnologias presentes no ambiente investigado.

\begin{itemize}
    \item \textbf{Intelligence Gathering} Esta etapa do pentest tem como objetivo levantar informações sobre o alvo, incluindo sistema operacional, frameworks utilizados, localização do servidor, presença de firewall, entre outros aspectos. Inicialmente, será utilizado o Google Dorks para identificar subdomínios, possíveis páginas antigas desatualizadas e arquivos ou credenciais expostas.
\end{itemize}
...
Com base nos Materiais descritos previamente nesta seção, a análise será executada visando obter informações sobre o alvo da istituição, levantar evidencias e relatar eventuais problemas na segurança do site.
...
\subsection{Reconhecimento e coleta de informções}

Esta etapa do processo de pentes, busca obter o máximo de informações possíveis sobre o alvo, desde informações referentes à arquitetura do ecossitema institucional, à profissionais que trabalham dentro da instiuição. 
...
Com o objetivo de identificar tecnologias, serviços expostos e possíveis características da infraestrutura utilizada pela aplicação institucional da Atitus, foram utilizadas diversas ferramentas amplamente utilizadas no processo de Pentest.
...
Teno isso em vista, ferramentas como o Whois do registro.br, trazem varias informações pertinentes ao processo de Intelligence Gathering. 
...
Durante a etapa de enumeração DNS, foi utilizada a ferramenta dnsenum com o objetivo de coletar informações relacionadas à infraestrutura do domínio atitus.edu.br, incluindo registros DNS, servidores de nomes (NS) e servidores de e-mail (MX). Apesar de alguns parâmetros informados no comando terem retornado erro de sintaxe (Value "-f" invalid for option s e Unknown option: i), a ferramenta conseguiu realizar parcialmente a enumeração do domínio. Como resultado, foram identificados múltiplos endereços IP associados ao domínio principal, todos pertencentes à infraestrutura da Cloudflare, reforçando a presença de proteção WAF no ambiente. Também foram identificados os servidores DNS autoritativos alexis.ns.cloudflare.com e ines.ns.cloudflare.com, reforçando a utilização da Cloudflare como intermediária da infraestrutura web. Além disso, a enumeração revelou que o serviço de e-mail da instituição utiliza a infraestrutura do Google Workspace, evidenciada pelos registros MX apontando para aspmx.l.google.com e demais servidores secundários do Google. Essas informações auxiliam no mapeamento inicial da superfície de ataque e na compreensão da arquitetura tecnológica utilizada pelo domínio analisado.
...
Posteriormente, durante a análise autenticada do portal institucional através das ferramentas de desenvolvedor do navegador Google Chrome, foram identificados os cookies .ASPXAUTH e ASP.NET SessionId, amplamente associados a aplicações desenvolvidas utilizando ASP.NET sobre o .NET Framework. Além disso, o header X-Powered-By: ASP.NET reforçou a hipótese de utilização da stack Microsoft no backend da aplicação. O cookie .ASPXAUTH é tradicionalmente empregado em mecanismos de autenticação Forms Authentication do ASP.NET, indicando gerenciamento de sessão baseado em cookies autenticados no lado servidor.
% Figura
\begin{figure}[H]
    \label{fig:id_figura}
    \centering
